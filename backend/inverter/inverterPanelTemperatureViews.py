...
class InverterPanelTemperatureView(APIView):
	...
	while staticDataInitDone == "FALSE":
		...
		if connection_status == 200 and (connection_status != 'Error while connecting to PostgreSQL' or connection_status != 'Errors encountered!'):
			...
			queryStatement = "select min_panel_temp_1,max_panel_temp_1,min_panel_temp_2,max_panel_temp_2,equipment_type from "+config.INVERTER_RANGE+""
			parameter = []
	...
	def get (self, request, *args, **kwargs):
		...
		queryStatement = "select panel_temp_1,panel_temp_2,equipment_type from "+config.INVERTER_THRESHOLD+""
		parameter = []
		...
		if whichtype == 'panel-temp1':
			...
			panelTemp1Dict = {"type":"panel temperature 1","data_series":[],"mark_lines":""}
			...
			queryStatement = "select station_id,system_id,subsystem_id,detail_code,panel_temp_1,record_time from "+config.INVERTER_DATA+" order by station_id,system_id,subsystem_id,detail_code,record_time ASC"
			parameter = []
			...
			panelTemp2Dict = {"type":"panel temperature 2","data_series":[],"mark_lines":""}
			...
			queryStatement = "select station_id,system_id,subsystem_id,detail_code,panel_temp_2,record_time from "+config.INVERTER_DATA+" order by station_id,system_id,subsystem_id,detail_code,record_time ASC"
			parameter = []
			resultList = queryPostgre(queryStatement,parameter)
