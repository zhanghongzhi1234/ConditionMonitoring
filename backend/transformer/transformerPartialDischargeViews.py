...
class TransformerPartialDischargeView(APIView):
	...
	while staticDataInitDone == "FALSE":
		...
		if connection_status == 200 and (connection_status != 'Error while connecting to PostgreSQL' or connection_status != 'Errors encountered!'):
			...
			queryStatement = "select acronym_asset_name,equipment_category,equipment_type,equipment_type_name,station_id,system_id,subsystem_id,detail_code,manufacturer,child_entity from "+config.EQUIPMENT_INFO+"  where equipment = 'transformer' order by acronym_asset_name"
			parameter = []
